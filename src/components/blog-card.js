...
const BlogCard = ({ blog }) => {
    ...
    return (
        <Card className={classes.root}>
            <Link to={`/blog/${title.replace('?','')}`}><Box className={classes.image} /></Link>
            <Grid container direction="column" justify="center" alignItems="center">
                <Grid container direction="column" justify="space-between" alignItems="center">
                    <Link to={`/blog/${title.replace('?','')}`}><Typography className={classes.title} variant="h2">{title}</Typography></Link>
